...
SELECT b.book_id,  b.title, a.name, c.category_name, b.available_copies
FROM Books b
...
UPDATE Books SET available_copies = available_copies +1, status = 'Available'
WHERE book_id = 3;

-- Check total books per category
SELECT c.category_id, c.category_name, CASE WHEN SUM(b.total_copies) >0 THEN SUM(b.total_copies) ELSE 0 END AS total_books FROM Category c 
LEFT JOIN Books b ON b.category_id =c.category_id
GROUP BY c.category_id;

-- search book_id by title
SELECT book_id, title
FROM Books WHERE title LIKE '%Harry Potter%';
...
SELECT b.book_id, b.title, c.category_name, a.name, b.available_copies
FROM Books b
JOIN Category c on b.category_id = c.category_id
JOIN Author a on b.author_id = a.author_id
...
SELECT book_id, title, CASE WHEN available_copies >3 THEN 'In Stock'
